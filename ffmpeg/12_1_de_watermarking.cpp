...
//去水印函数
int remove_watermark(const char *input_file, const char *output_file) {
    // 注册所有组件
    // av_register_all();
    ...
    char crop_filter_args[256];
    snprintf(crop_filter_args, sizeof(crop_filter_args), "crop=in_w-10:in_h-10:10:10");

    // 解析过滤器描述
    if (avfilter_graph_parse2(filter_graph, crop_filter_args, &inputs, &outputs) < 0) {
...
//TODO，等待过程验证
int main(){
    const char *input_file = "your_video_file_with_watermark.mp4";
    const char *output_file = "your_video_file_without_watermark.mp4";
    int result = remove_watermark(input_file, output_file);
    if (result == 0) {
        printf("去除水印成功\n");
    } else {
        printf("去除水印失败\n");
